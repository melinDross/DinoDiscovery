...
**Drag-to-rotate**: dragging the card (`onTouchStart`/`onTouchMove`/`onTouchEnd` on `.card-perspective`) rotates it live with the finger on **both axes** — horizontal drag drives `spinDeg` (rotateY, `DRAG_SENSITIVITY_DEG_PER_PX = 0.6`), vertical drag drives `tilt.rotateX`, both with `transition: 'none'` so the rendered rotation always matches the touch position exactly, no easing lag. Only the horizontal axis snaps on release (to whichever face — front/back, nearest multiple of 180° — ended up closer, animated over `DRAG_SNAP_TRANSITION`); the vertical axis always springs back to level (`0deg`), same as the pre-existing hover/light-touch parallax tilt, because the back face's art is only oriented correctly for a Y-axis flip — there's no sensible "back" to snap to on X. Because the rotation is driven directly by state during a drag (no CSS-interpolated unknown), `facing` is computed synchronously from the live angle on every `touchmove` rather than via the `setTimeout` approximation used for the button-triggered spin/flip-in (where the actual visual angle is CSS-interpolated and JS can't read it without polling — the timer approach there is a deliberate, separate technique, not a leftover to consolidate).
