...
## ✅ Post-audit follow-up: html2canvas replaced with modern-screenshot (2026-07-02, fixed & verified)
...
**Verified 2026-07-02, on the real deployed site (v0.6.3):** user confirmed the downloaded card renders perfectly, including the rotated habitat tag and rounded corners that html2canvas previously got wrong. This satisfies the same standard both prior html2canvas bugs were caught against (actual capture inspection, not just live-browser/unit-test checks). Note this doesn't change the separate, still-open item below (dino 3D pop-out clipping at the capture's bounding box) — that's a bounding-box behavior any DOM-capture library shares, not something either library's rendering-fidelity difference fixes.
